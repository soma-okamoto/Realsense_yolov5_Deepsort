...
curr_frames, prev_frames = [None] * 1, [None] * 1
...
class Yolov5Detector:
    ...
    def dectshow(self,color_image,depth_image, color_intr):
        main(depth_image,color_image,color_intr)
    ...
    def ImageCallback(self, depthdata, colordata):
        ...
        pubdata.header.stamp = colordata.header.stamp  #同期取るときに必要
        try:
        ...
        self.dectshow(color_image,depth_image, color_intr)
...
def run(
    ...
    try:
        ...
        # for path, im, im0s, vid_cap, s in dataset:
            ...
            t1 = time_sync()
            im = torch.from_numpy(im).to(deviceType)
            ...
            # NMS
            pred = non_max_suppression(pred, conf_thres_start, iou_thres_start, classes_start, agnostic_nms_start, max_det=max_det_start)
            dt[2] += time_sync() - t3
                    ...
                    if len(outputs[i]) > 0:
                        ...
                        for j, (output, conf) in enumerate(zip(outputs[i], confs)):
                            ...
                            if save_img or save_crop or view_img:  # Add bbox to image
                                ...
                                zDepth = depth[int(output[1]),int(output[0])]
                                ...
                                cv2.drawMarker(depth,position=(d1, d2),color=(0, 255, 0),markerType=cv2.MARKER_STAR,markerSize=20,thickness=2,line_type=cv2.LINE_4)
                                ...
                                detect_box.append(float(output[3]))
                        ...
                        box_array_forPublish = Float32MultiArray(data=detect_box)
                        pubdata.yolo_box_data = box_array_forPublish
                        #pubdata.header.stamp = rospy.Time.now() #同期とるときには消す
    ...
    if update:
        strip_optimizer(weights_model)  # update model (to fix SourceChangeWarning)
...
def main(depth_image,color_image,color_intr):
    check_requirements(exclude=('tensorboard', 'thop'))
    run(depth_image = depth_image,color_image = color_image,color_intr = color_intr)
...
def start(        
    ...
    dnn=False,  # use OpenCV DNN for ONNX inference
        ...
        global conf_thres_start
        conf_thres_start = conf_thres

        global iou_thres_start
        iou_thres_start = iou_thres

        global classes_start
        classes_start = classes

        global agnostic_nms_start 
        agnostic_nms_start = agnostic_nms

        global max_det_start
        max_det_start = max_det

        global strongsort_list
        strongsort_list = []

        global weights_model
        ...
        # Load model
        global deviceType
        deviceType = select_device(device)
        global model
        model = DetectMultiBackend(weights=os.path.join(os.getcwd(),'yolov5s.pt'), device=deviceType, dnn=dnn, data=data, fp16=half)
        global stride,names,pt
